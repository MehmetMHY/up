...
printf "🐦 ${GREEN}SNAP Refresh${END} \n"
sudo snap refresh

if [ "$?" -ne 0 ]; then
    echo "E: failed to refresh snap packages. Aborting..."
    exit 1
fi
...
if [[ "$1" == "-a" ]]; then
    ...
    printf "${ORANGE}➤ SNAP Pkgs INFO:${END}\n"
    echo $(expr $(snap list | wc -l) - $(echo 1))" packages installed"
    if [ "$?" -ne 0 ]; then
        echo "E: failed to get information on snap packages. Aborting..."
        exit 1
    fi
